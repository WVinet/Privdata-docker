...
export function AppLayout() {
  const { isAuthenticated, getUser, hasRole } = useAuth()
      ...
      <div className="flex flex-col flex-1 overflow-hidden min-w-0">
        <header className="h-14 border-b border-border flex items-center px-4 md:px-6 shrink-0 bg-card gap-3">
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-foreground">Agencia</span>
          </div>
          ...
          <p className="hidden md:block text-sm text-muted-foreground">
            Agencia de Protección de Datos Personales — Panel del Auditor
          </p>
        ...
        <footer className="border-t border-border px-4 md:px-6 py-3 text-xs text-muted-foreground shrink-0 bg-card">
          Agencia de Protección de Datos Personales · Sistema externo de fiscalización — Ley 21.719
        </footer>
      </div>
    </div>
  )
